...
## 10. Related Observation: CSP `unsafe-inline`

The response CSP on `https://compute.ornn.com` includes `'unsafe-inline'` in
`script-src` (and `style-src`). Per
[content-security-policy.com/unsafe-inline](https://content-security-policy.com/unsafe-inline/),
this keyword "annuls most of the security benefits that Content-Security-Policy
provide" — it allows inline `<script>` blocks and inline event handlers
(`onclick`, etc.) to execute.

**XSS is not currently possible.** Every input tested so far is correctly
output-encoded, so there is no injection point to place inline script into the
page. This is a defense-in-depth gap, not an active vulnerability: if any future
endpoint (search box, profile field, API error string) misses encoding, CSP
would normally contain it — with `unsafe-inline` it will not.

Still investigating whether any endpoint reflects unescaped markup that would
let `unsafe-inline` be exercised end to end. Will update this section if one is
found.

**Recommendation:** drop `'unsafe-inline'` and move to a per-response
nonce-based CSP (or `'strict-dynamic'` + nonce, which makes modern browsers
ignore `unsafe-inline` outright).
